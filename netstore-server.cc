...
namespace netstore {
    ...
    void work_receive(int tcp_sock, int fd, size_t file_size, const char *filename) {
        ...
        int sock_fd = -1;
        fd_set rfds;
        std::filesystem::path file_node(std::string(s_config.shared_folder + '/' + filename));
    ...
    bool do_send(int sock, struct SIMPL_CMD *request, size_t req_len, std::string client_ip, int client_port) {
...

        {
            ...
            if (file_pos == filenames_end) {
                printf(msg_pckg_error, client_ip.c_str(), client_port, "file not hosted on the server");
                return false;
            }
        }
    ...
    bool do_receive(int sock, struct CMPLX_CMD *request, size_t req_len, std::vector<std::string> &filenames) {
        ...
        snprintf(no_way.data, SIMPL_CMD_DATA_SIZE, "%s", request->data);
        ...
        std::vector<std::string>::iterator filenames_end;
        {
            mutex_files.lock();
            file_pos = std::find(filenames.begin(), filenames.end(), filename);
            filenames_end = filenames.end();
            mutex_files.unlock();
        }

        if ((int64_t) file_size > s_config.free_space || filename.find('/') != std::string::npos
            || filename.length() == 0 || file_pos != filenames_end
            || (fd = open(filepath.c_str(), O_WRONLY | O_CREAT | O_EXCL, S_IRWXU | S_IRWXG | S_IRWXO)) == -1) {
            cmd_send(sock, &no_way, EMPTY_SIMPL_CMD_SIZE + data_len, &client_address);
            return false;
        }
        ...
        memcpy(response.data, request->data, data_len);
    ...
    for (;;) {
        ...
        switch (parse_req_type(&buffer, msg_len)) {
            ...
            case req_type::download:
                do_send(sock, (struct SIMPL_CMD *) &buffer, msg_len, std::string(inet_ntoa(client_address.sin_addr)),
